...
A Connection Request for a Domain requires a `dns.resolve4()` or `dns.resolve6()` call
before, and on successfully resolved `A` or `AAAA` domain entries a connection to those
addresses.
...
that gradually checks for network timeouts and failure scenarios of the `net.connect()`
